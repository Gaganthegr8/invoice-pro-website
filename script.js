...
// Fires a GA4 event named "download" with which platform was chosen.
// Shows up in Google Analytics under Reports > Engagement > Events.
function trackDownload(platform) {
  if (typeof gtag === 'function') {
    gtag('event', 'download', {
      platform: platform,          // "mac" or "windows"
      file_name: platform === 'mac' ? 'Invoice-Pro-1.1.0.dmg' : 'Invoice-Pro-Setup-1.1.0.exe'
    });
  }
}
...
function setupDownloadButtons() {
  ...
  if (os === 'windows') {
    ...
    primaryBtn.onclick = () => { trackDownload('windows'); window.location.href = WIN_EXE; };

    secondaryLabel.textContent = 'Download for Mac';
    secondaryBtn.href = MAC_DMG;
    secondaryBtn.onclick = () => trackDownload('mac');
  } else {
    ...
    primaryBtn.onclick = () => { trackDownload('mac'); window.location.href = MAC_DMG; };

    secondaryLabel.textContent = 'Download for Windows';
    secondaryBtn.href = WIN_EXE;
    secondaryBtn.onclick = () => trackDownload('windows');
  }

  // The two big cards in the Download section further down the page
  const macCard = document.getElementById('mac-download-link');
  const winCard = document.getElementById('win-download-link');
  if (macCard) macCard.addEventListener('click', () => trackDownload('mac'));
  if (winCard) winCard.addEventListener('click', () => trackDownload('windows'));
}
